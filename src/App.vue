      ...
      <template v-else-if="store.activeTab === 'template'">
        ...
        <div class="ribbon-group-card">
          <div class="ribbon-group-body" style="display: grid; grid-template-rows: repeat(2, 28px); grid-auto-flow: column; gap: 4px;">
            <button
              class="btn"
              :class="(store.editorActions?.getActiveTab && store.editorActions.getActiveTab() === 'visual') ? 'btn-primary' : 'btn-secondary'"
              style="padding: 2px 8px; font-size: 0.72rem; height: 28px; grid-row: 1; display: inline-flex; align-items: center; gap: 4px;"
            ...
            <button
              class="btn"
              :class="(store.editorActions?.getActiveTab && store.editorActions.getActiveTab() === 'code') ? 'btn-primary' : 'btn-secondary'"
              style="padding: 2px 8px; font-size: 0.72rem; height: 28px; grid-row: 2; display: inline-flex; align-items: center; gap: 4px;"
        ...
        <div class="ribbon-group-card">
          <div class="ribbon-group-body" style="display: grid; grid-template-rows: repeat(2, 28px); grid-auto-flow: column; gap: 4px;">
            ...
            <button class="btn btn-secondary" style="padding: 2px 8px; font-size: 0.72rem; height: 28px; grid-row: 1; display: inline-flex; align-items: center; gap: 4px;" @click="store.editorActions?.openBlockModal && store.editorActions.openBlockModal('macro')" title="Insereix un bloc MACRO reutilitzable">
              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="16 18 22 12 16 6"/><polyline points="8 6 2 12 8 18"/></svg>
              <span v-if="store.config.showButtonTexts">MACRO</span>
            </button>
            <button class="btn btn-secondary" style="padding: 2px 8px; font-size: 0.72rem; height: 28px; grid-row: 2; display: inline-flex; align-items: center; gap: 4px;" @click="store.editorActions?.openBlockModal && store.editorActions.openBlockModal('set')" title="Insereix un bloc SET (captura contingut en una variable)">
              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="5" y1="9" x2="19" y2="9"/><line x1="5" y1="15" x2="19" y2="15"/></svg>
              <span v-if="store.config.showButtonTexts">SET</span>
            </button>
          </div>
          <div class="ribbon-group-label">ESTRUCTURA</div>
        </div>
        ...
        <div class="ribbon-group-card">
          <div class="ribbon-group-body" style="display: flex; align-items: center; height: 60px;">
            <div class="segmented-control" style="display: inline-flex; border: 1px solid var(--border-color); border-radius: 4px; padding: 1px; background: var(--bg-primary);">
              <button
                class="btn-segment"
                :class="{ active: store.previewViewMode === 'markdown' }"
                @click="store.setPreviewViewMode('markdown')"
                ...
                title="Mostra només el Markdown"
              >
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="8" y1="13" x2="16" y2="13"/><line x1="8" y1="17" x2="16" y2="17"/></svg>
                <span v-if="store.config.showButtonTexts">Markdown</span>
              </button>
              <button
                class="btn-segment"
                :class="{ active: store.previewViewMode === 'html' }"
                @click="store.setPreviewViewMode('html')"
                style="padding: 2px 8px; font-size: 0.72rem; border: none; background: transparent; cursor: pointer; border-radius: 3px; color: var(--text-primary); display: inline-flex; align-items: center; gap: 4px;"
                title="Mostra només l'HTML"
              >
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7Z"/><circle cx="12" cy="12" r="3"/></svg>
                <span v-if="store.config.showButtonTexts">HTML</span>
              </button>
              <button
                class="btn-segment"
                :class="{ active: store.previewViewMode === 'both' }"
                @click="store.setPreviewViewMode('both')"
                style="padding: 2px 8px; font-size: 0.72rem; border: none; background: transparent; cursor: pointer; border-radius: 3px; color: var(--text-primary); display: inline-flex; align-items: center; gap: 4px;"
                title="Mostra els dos panells"
              >
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="18" rx="1"/><rect x="14" y="3" width="7" height="18" rx="1"/></svg>
                <span v-if="store.config.showButtonTexts">Ambdós</span>
              </button>
            </div>
          </div>
          <div class="ribbon-group-label">VISTA</div>
      ...
      <div style="display: flex; align-items: center; gap: 4px; flex-shrink: 0; flex: 1; justify-content: center;">
        ...
        <template v-if="store.activeTab === 'template'">
          ...
          <template v-if="!store.editorActions?.getActiveTab || store.editorActions.getActiveTab() === 'visual'">
            ...
            <button class="btn-secondary" style="padding: 2px 6px; font-size: 0.72rem; height: 26px; display: inline-flex; align-items: center;" @click="store.editorActions?.openBlockModal && store.editorActions.openBlockModal('macro')" title="Insereix un bloc MACRO reutilitzable"><svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="16 18 22 12 16 6"/><polyline points="8 6 2 12 8 18"/></svg></button>
            <button class="btn-secondary" style="padding: 2px 6px; font-size: 0.72rem; height: 26px; display: inline-flex; align-items: center;" @click="store.editorActions?.openBlockModal && store.editorActions.openBlockModal('set')" title="Insereix un bloc SET (captura contingut en una variable)"><svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="5" y1="9" x2="19" y2="9"/><line x1="5" y1="15" x2="19" y2="15"/></svg></button>
