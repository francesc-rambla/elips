  ...
  for (let idx = 0; idx < tokens.length; idx++) {
    ...
    if (matchTag) {
      ...
      if (expr.startsWith('for ')) {
        ...
        if (isBlock) {
          htmlResult += `<div class="jinja-block" contenteditable="false" data-layout="block" data-type="for" data-cond="${cond}"><div class="j-head" data-type="for"><div style="display:flex;align-items:center;gap:4px;">🔁 <span style="font-weight:700;color:var(--color-primary);">PER CADA:</span> <span class="j-cond-text" data-cond="${cond}">${cond}</span></div><div class="j-actions"><button class="j-btn-mini btn-layout" style="background-color:var(--color-primary);color:white;border:none;" title="Canvia a mode integrat al text (Inline)">🌐 Inline</button><button class="j-btn-mini btn-trash" style="background-color:var(--color-danger);color:white;border:none;" title="Elimina el bucle">🗑️</button></div></div><div class="j-content" contenteditable="true">`;
          stack.push('for');
        } else {
        ...
        if (isBlock) {
          htmlResult += `<div class="jinja-block" contenteditable="false" data-layout="block" data-type="if" data-cond="${cond}"><div class="j-head" data-type="if"><div style="display:flex;align-items:center;gap:4px;">🔀 <span style="font-weight:700;color:#b45309;">SI:</span> <span class="j-cond-text" data-cond="${cond}">${cond}</span></div><div class="j-actions"><button class="j-btn-mini btn-layout" style="background-color:var(--color-primary);color:white;border:none;" title="Canvia a mode integrat al text (Inline)">🌐 Inline</button><button class="j-btn-mini btn-elif" title="Afegeix branca O SI (ELIF)">+ ELIF</button><button class="j-btn-mini btn-else" title="Afegeix branca EN CAS CONTRARI (ELSE)">+ ELSE</button><button class="j-btn-mini btn-trash" style="background-color:var(--color-danger);color:white;border:none;" title="Elimina el condicional">🗑️</button></div></div><div class="j-content" contenteditable="true">`;
          stack.push('if');
        } else {
        ...
        if (parentType === 'if-inline') {
          ...
          htmlResult += `</div><div class="j-branch" data-type="elif"><div style="display:flex;align-items:center;gap:4px;">↪️ <span style="font-weight:700;color:#b45309;">O SI:</span> <span class="j-cond-text" data-cond="${cond}">${cond}</span></div><button class="j-btn-mini btn-branch-trash" style="background-color:var(--color-danger);color:white;border:none;" title="Elimina la branca">🗑️</button></div><div class="j-content" contenteditable="true">`;
        }
      } else if (expr === 'else') {
        const parentType = stack[stack.length - 1];
        if (parentType === 'if-inline') {
