...
const openVisualGridEditor = () => {
  ...
  isVisualGridModalOpen.value = true;
};
...
const saveVisualGridEditor = () => {
  visualGridRows.value.forEach((row, rIdx) => {
    ...
    row.items.forEach((item, fIdx) => {
      ...
      const target = groupConfigList.value.find(g => g.element === item.element);
      if (target) {
  ...
  isVisualGridModalOpen.value = false;
  store.addLog(`Distribució visual de les files i camps actualitzada per al grup '${activeConfigGroup.value}'.`, 'info');
};
...
<template>
  <div class="workspace-wrapper">
    ...
    <div class="modal-overlay" v-if="isVisualGridModalOpen" style="display: flex; z-index: 1200;">
